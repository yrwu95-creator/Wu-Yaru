...
st.markdown(f"🕒 日本时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
today = date.today()
days_left = max((spring_festival_2027 - today).days, 0)
...
with col1:
    ...
    if st.button("🍜 好好吃饭", use_container_width=True):
        st.session_state.messages.append("✨ 收到！要吃嘛嘛香，营养均衡~")
        st.balloons()
